...
const VALID_STATUSES = ["submitted", "approved", "denied", "reimbursed"] as const;
  ...
  try {
    ...
    const baseQuery = db
      .select({
        ...
        urgentReimbursementNeeded: reimbursementRequests.urgentReimbursementNeeded,
    ...
    const rows =
      filters.length > 0
        ? await baseQuery.where(and(...filters)).orderBy(desc(reimbursementRequests.submittedAt))
...
router.patch("/:id/status", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    if (!nextStatus || !VALID_STATUSES.includes(nextStatus as (typeof VALID_STATUSES)[number])) {
